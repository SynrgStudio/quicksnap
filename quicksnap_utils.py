...
def is_linked_asset(obj):
    """
    Check if an object is a linked asset from the Asset Browser.
    Returns True if the object is linked or is an instance of a linked collection.
    """
    # Check if object is directly linked
    if obj.library is not None:
        return True

    # Check if object is an instance of a linked collection
    if obj.instance_type == 'COLLECTION' and obj.instance_collection is not None:
        if obj.instance_collection.library is not None:
            return True
...
def is_readonly_asset(obj):
    """
    Check if an object is read-only (linked assets cannot be modified).
    """
    if obj.library is not None:
        return True

    # Collection instances with linked collections are also read-only
    if obj.instance_type == 'COLLECTION' and obj.instance_collection is not None:
        if obj.instance_collection.library is not None:
            return True
...
def get_safe_evaluated_object(obj, depsgraph, ignore_modifiers=False):
    """
    Safely get evaluated object, handling linked assets properly.
    For linked assets, avoid evaluated_get() when possible to prevent issues.
    """
    try:
        # For linked assets, prefer original geometry when modifiers are ignored
        if ignore_modifiers or is_linked_asset(obj):
            return obj
        else:
            # Try to get evaluated object, but handle potential errors
            evaluated_obj = obj.evaluated_get(depsgraph)
            return evaluated_obj
    except Exception as e:
        logger.warning(f"Failed to get evaluated object for {obj.name}: {e}")

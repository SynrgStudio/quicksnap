...
class QuickVertexSnapOperator(bpy.types.Operator):
    ...
    def terminate(self, context, revert=False):
        ...
        # Clean up wireframe display for all objects that might have been affected
        self.cleanup_wireframe_display()
    ...
    def cleanup_wireframe_display(self):
        """
        Clean up wireframe display for all objects that might have been affected during the operation.
        This is especially important after duplication operations where duplicated objects
        inherit the wireframe state but are not in the backup system.
        """
        # Get all objects that were in our backup (original objects we modified)
        for obj_name in self.target_object_display_backup:
            if obj_name in bpy.data.objects:
                obj = bpy.data.objects[obj_name]
                # Revert to original state
                (obj.show_wire, obj.show_name, obj.show_bounds, obj.display_bounds_type) = \
                    self.target_object_display_backup[obj_name]

        # Additionally, clean up any objects that might have wireframe enabled
        # but weren't in our original backup (like duplicated objects)
        for obj in bpy.data.objects:
            # Only clean up objects that are visible and might have been affected
            if not obj.hide_get() and obj.show_wire:
                # Check if this object was created during our operation
                # by checking if it's not in our backup but has wireframe enabled
                if obj.name not in self.target_object_display_backup:
                    # This could be a duplicated object or an object that got wireframe
                    # enabled during the operation. Let's disable wireframe.
                    obj.show_wire = False
                    obj.show_name = False
